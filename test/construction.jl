...
@testset verbose = true "Construction" begin
	...
	@testset "now(Timestamp64)" begin
		ts = now(Timestamp64)
		dt = Dates.now() # note that Dates.now() returns local system time!
		@test abs(dt - DateTime(ts)) < Millisecond(5)
	end
	...
	@testset "today(Timestamp64)" begin
		...
		dt = Dates.today() # note that Dates.today() return local system time!
		@test DateTime(ts) == dt
	end

	@testset "today(Timestamp64, UTC)" begin
